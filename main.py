...
def main(page: ft.Page):
    ...
    def get_header():
        ...
        menu = ft.PopupMenuButton(
            ...
            items=[
                ...
                ft.PopupMenuItem(content=ft.Text("חברים שלי"), data="friends", on_click=handle_menu),
    ...
    def build_screen(screen):
        ...
        if screen == "welcome":
            ...
            upload_btn = ft.Button(content=ft.Text("העלה מהגלריה"), icon=ft.Icons.PHOTO_LIBRARY, on_click=lambda _: file_picker.pick_files(allow_multiple=False, file_type=ft.FilePickerFileType.IMAGE), bgcolor=ft.Colors.WHITE)
            ...
            if not favorites:
                ...
                for act in backend.activities_db.values():
                    if act['name'] in favorites:
                        ...
                        fav_col.controls.append(ft.Container(content=ft.Row([ft.Text(f"❤️ {act['name']}", size=16, color="#4B0082", weight=ft.FontWeight.BOLD), ft.Text(f" - {act['price']} ₪", size=16)]), on_click=make_click(act), ink=True, padding=8, border_radius=8, bgcolor=ft.Colors.BLUE_50))
            ...
            events_main_section = ft.Container(
                content=ft.Column([
                    ft.Text("האירועים וההזמנות שלי", size=22, weight=ft.FontWeight.BOLD, color="#4B0082"),
                    ft.Divider(),
                    events_col
                ]),
            ...
            inv_people = {v: k for k, v in backend.people_map.items()}
            inv_budget = {v: k for k, v in backend.budget_options.items()}
            people_dd = ft.Dropdown(options=[ft.dropdown.Option(key=k, text=k) for k in backend.people_map.keys()], value=inv_people.get(backend.user_session['people_count'], '3-5'), width=300, bgcolor=ft.Colors.WHITE)
            budget_dd = ft.Dropdown(options=[ft.dropdown.Option(key=k, text=k) for k in backend.budget_options.keys()], value=inv_budget.get(backend.user_session['budget'], 'ללא הגבלה'), width=300, bgcolor=ft.Colors.WHITE)
            area_dd = ft.Dropdown(options=[ft.dropdown.Option(key=a, text=a) for a in backend.areas], value=backend.user_session['area'], width=300, bgcolor=ft.Colors.WHITE)
            ...
            sort_dd = ft.Dropdown(options=[ft.dropdown.Option(key='default', text='ברירת מחדל'), ft.dropdown.Option(key='price_asc', text='מחיר: נמוך לגבוה'), ft.dropdown.Option(key='price_desc', text='מחיר: גבוה לנמוך')], value=backend.user_session['sort_by'], label="מיון", width=150, bgcolor=ft.Colors.WHITE)
            people_dd = ft.Dropdown(options=[ft.dropdown.Option(key=k, text=k) for k in backend.people_map.keys()], value=inv_people.get(backend.user_session['people_count'], '3-5'), label="אנשים", width=120, bgcolor=ft.Colors.WHITE)
            budget_dd = ft.Dropdown(options=[ft.dropdown.Option(key=k, text=k) for k in backend.budget_options.keys()], value=inv_budget.get(backend.user_session['budget'], 'ללא הגבלה'), label="תקציב", width=150, bgcolor=ft.Colors.WHITE)
            area_dd = ft.Dropdown(options=[ft.dropdown.Option(key=a, text=a) for a in backend.areas], value=backend.user_session['area'], label="איזור", width=120, bgcolor=ft.Colors.WHITE)
            ...
            popup_box = ft.Container(
                content=ft.Column([
                    ft.Text(f"יצירת אירוע - {item['name']}", size=20, weight=ft.FontWeight.BOLD),
                    date_time_input,
                    optional_text_input,
                    ft.Text("להזמין חברים?"),
                    ft.Container(content=friends_list_ui, padding=10, bgcolor=ft.Colors.BLUE_50, border_radius=8),
                    ft.Row([
                        ft.Button("ביטול", on_click=hide_popup),
                        ft.Button("צור אירוע", on_click=submit_event, bgcolor=ft.Colors.GREEN, color=ft.Colors.WHITE)
                    ])
                ], tight=True),
                bgcolor=ft.Colors.WHITE, padding=25, border_radius=15, width=350
            )
            ...
            sticky_bottom_bar = ft.Container(content=ft.Row([ft.Button(content=ft.Text("📅 צור אירוע / הזמן מקום", size=18, weight=ft.FontWeight.BOLD), bgcolor=ft.Colors.GREEN, color=ft.Colors.WHITE, height=60, expand=True, on_click=lambda _: show_popup(popup_box)), fav_btn], spacing=10), bgcolor=ft.Colors.BLUE_50, padding=20, border_radius=25)
